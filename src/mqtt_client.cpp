...
bool MqttClient::publish(int &mid, const std::string &topic, const void *payload, int payload_len, int qos) {
    ...
    spdlog::debug("发送[{}]MCU消息[{}]至主题[{}]QOS[{}]", mid, base64_payload, topic, qos);
...
bool MqttClient::connect() {
    ...
    if (rc != MOSQ_ERR_SUCCESS) {
        spdlog::warn("重置客户端ID[{}]失败", client_id_);
        return false;
    }
    spdlog::info("设置用户名密码");
...
bool MqttClient::subscribe_topic(int &mid, const std::string &topic, int qos) {
    if (!is_connected_) {
        return false;
    }
    if (topic.empty()) {
